  ...
  return (
      <div className='box'>
        ...
        <div className='relative w-full h-80 rounded-lg overflow-hidden group cursor-pointer mb-5'>
          <Image src={currentImage} alt={portfolio.title} fill className='object-cover'/>
          <div className='bg-black bg-opacity-10 backdrop-blur-[1px] absolute top-0 left-0 w-full h-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center'>
            <div className='w-16 h-16 flex items-center justify-center bg-peach-700 border border-paech-400 p-3 text-grey-700 rounded-lg'>
              <ArrowUpRight />
            </div>
          </div>
        </div>
        <div className='grid grid-cols-3 gap-3'>
          {portfolio.images.map((image, index) => (
              <div key={index} className='relative h-20 w-full rounded-lg overflow-hidden group cursor-pointer' onClick={() => setCurrentImage(image)}>
                <Image src={image} alt={portfolio.title} fill className='object-cover'/>
                <div
                    className={`bg-black bg-opacity-10 backdrop-blur-[1px] absolute top-0 left-0 w-full h-full transition-opacity duration-300 ${currentImage !== image ? 'opacity-100' : 'opacity-0'}`}>
